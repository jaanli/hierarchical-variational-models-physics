...
class MaskedLinear(nn.Linear):
    def __init__(self, in_channels, out_channels, n, bias, exclusive, use_context=False):
        ...
        if use_context:
            self.cond_linear = nn.Linear(n, out_channels * n, bias=False)

        # Correction to Xavier initialization
        self.weight.data *= torch.sqrt(self.mask.numel() / self.mask.sum())

    def forward(self, x, context=None):
        out = nn.functional.linear(x, self.mask * self.weight, self.bias)
        if hasattr(self, 'cond_linear'):
            out += self.cond_linear(context)
        return out
...
class ChannelMADE(nn.Module):
    def __init__(self, **kwargs):
        super(ChannelMADE, self).__init__()
        #self.L = kwargs['L']
        ...
        if self.net_depth >= 2:
            # out_channels is 2 for the output (mu and alpha)
            layers.append(self._build_simple_block(self.net_width, 2))
    ...
    def forward(self, x, context=None):
        ...
        hidden = self.first_layer(x, context)
        x_hat = self.net(hidden)
